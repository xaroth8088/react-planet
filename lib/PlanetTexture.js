...
export default class PlanetTexture {
    constructor({
        ...
        this.landNoise = landNoise;
        this.spin = spin;
        this.landColor1 = landColor1;
        this.landColor2 = landColor2;
        this.width = width;
        this.height = width / 2;
        this.waterLevel = waterLevel;
        this.waterFalloff = waterFalloff;
        this.waterDeep = waterDeep;
        this.waterShallow = waterShallow;
        this.waterSpecular = waterSpecular;
        this.cloudNoise = cloudNoise;
        this.cloudOpacity = cloudOpacity;
        this.cloudColor = cloudColor;

        this.iterator = new XYIterator(width, this.height);
        this.diffuse = new PixelSurface(width, this.height);
        this.normal = new PixelSurface(width, this.height);
        this.specular = new PixelSurface(width, this.height);
        this.cloud = new PixelSurface(width, this.height);
        this.done = false;
    }

    surfaceHeight = (x, y, z) => this.surfaceNoise.sample(x / this.spin, y / this.spin, z);

    surfaceColor = (x, y, z) => {
        const c = this.landNoise.sample(
            x / this.spin,
            y / this.spin,
            z
        );
        const q0 = c;
        const q1 = 1 - c;
        const r = this.landColor1.r * q0 + this.landColor2.r * q1;
        const g = this.landColor1.g * q0 + this.landColor2.g * q1;
        const b = this.landColor1.b * q0 + this.landColor2.b * q1;
        return {
            r,
            g,
            b
    ...
    update = () => {
        ...
        const {
            width, height, cloud, iterator, surfaceHeight, surfaceColor, diffuse, specular, normal,
