...
void TextureGenerator::init() {
    ...
    landNoise = new NoiseWrapper(landSeed, landiScale, landiOctaves, landiFalloff,
                                 landiIntensity, landiRidginess, landsScale,
                                 landsOctaves, landsFalloff, landsIntensity);
    ...
    for (unsigned int x = 0; x < width; x++) {
        for (unsigned int y = 0; y < height; y++) {
            ...
            if (c0 > waterLevel) {
                ...
                XYZ n = normalizedCrossProduct(1.0 / double(width), 0.0, (cx - c0), 0.0,
                                               1.0 / double(height), (cy - c0));
                
                RGB normalPixel = normalRGB(n.x, -n.y, n.z);
                setPixel(normalBuffer, x, y, normalPixel);
            } else {
                // For the "below water" case, there's no additional sampling - we
                // simply blend the shallow and deep water colors based on how deep the
                // water is at this point.
